...
class RoboConUSBCamera(Camera):
    ...
    def res(self, new_res):
        if new_res is not self._res:
            ...
            self._res = new_res
        self._update_camera_params(self.focal_lengths) # updates focal lengths even if resolution unchanged, to avoid confusing user behaviour when focal lengths changed
